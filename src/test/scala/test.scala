...
class TestParse extends FunSuite {
  ...
  test("test2") {
    assert(OpegParser.parse(new FileReader("src/test/resources/GPEG/test2.gpeg")).toString == "Grammar(Pos(1,1),'A,List(Rule(Pos(1,1),'A,AnyNonterminal(Pos(1,6),'B)), Rule(Pos(2,1),'B,Str(Pos(2,6),Hello))))")
  }
