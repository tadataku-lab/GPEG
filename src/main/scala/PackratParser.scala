...
object PackratParser{  
    ...
    class PackratParser(){
        ...
        def exe_match(p: ParserContext, bytes: Array[Byte]): TreeSet[Int] 
        = p.map_pos(bytes).result.positions

        private[this] val map_call: (ParserContext, Int) => TreeSet[Int] = 
        (p: ParserContext, nsym: Int) => {
            val prev_positions = p.result.positions.clone
            val prev_trees = p.result.trees.clone
            val new_result = p.new_result(TreeSet())
        ...
        (p: ParserContext, nsym: Int, pos: Int)
        => parse(p.set_exp(p.rules(nsym)).set_result(p.new_result(TreeSet(pos)))).memo(nsym, pos)

        def map_union(p: ParserContext, lhs: PExp, rhs: PExp): TreeSet[Int] = {
            p.result.positions = p.result.positions.flatMap(pos => union(p, lhs, rhs, pos))
            p.result.positions
        }

        def union(p: ParserContext, lhs: PExp, rhs: PExp, pos: Int): TreeSet[Int] = {
            val prev_trees = p.result.trees.clone
        ...
        def merge(p: ParserContext, lhs_result: ParserResult, rhs_result: ParserResult): TreeSet[Int] = {
            (lhs_result.positions.nonEmpty, rhs_result.positions.nonEmpty) match{
                ...
                case (false, false) => p.set_result(p.new_result(TreeSet())).result.positions
            }
        }
        ...
        (p: ParserContext) => {
            p.exp match {
                ...
                case PMatch(bytes, next) => if(!exe_match(p, bytes).nonEmpty) p.set_exp(PFail("")).set_result(p.new_result(TreeSet())) else parse(p.set_exp(next))
                case PCallNum(nsym, next) => if(!map_call(p, nsym).nonEmpty) p.set_exp(PFail("")).set_result(p.new_result(TreeSet())) else parse(p.set_exp(next))
                ...
                */
                ...
                case PNot(body, next) => TreeSet
                    ...
                    new_p match {
                        ...
                        case AmbContext(lhs, rhs, _) => {
                            lhs.TreeSetExp(PFail("Match PExp: " + body))
                            rhs.TreeSetExp(PFail("Match PExp: " + body))
                            amb_parse(tree, new_p)
                        }
                    }
                }

                case PAnd(body, next) => TreeSet
                    p.exp = body
                    val (new_tree, new_p) = parse(tree, p.copy)
                    new_p match {
                        case p_c: ParserContext => {
                ...
                case PMany(body, next) => TreeSet
                    p.exp = body
                    val (new_tree, new_p) = many(tree, p)
                    amb_parse(new_tree, new_p.TreeSetExp(next))
                }

                case PFold(name, body, rec, next) => TreeSet
                    if(p.folding){
                        p.exp = PFold(name, body, rec, next)
                        (tree, p)
                    }else{
                        val (new_tree, new_p) = fold(name, body, rec, tree, p.TreeSetFold(true))
                        amb_parse(new_tree, new_p.TreeSetExp(next).TreeSetFold(false))
                    }
                }
                */
            }
